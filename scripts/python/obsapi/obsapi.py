...
class OBSAPI(api.API):
    ...
    def get_token(self, cookiejar):
        ...
        if not cookiejar:
            cookiejar = filename=os.environ['HOME'] + '/.local/state/osc/cookiejar'
    ...
    def package_exists(self, project, package):
        return self.check_exists('/'.join(['/source', project, package, '_meta']))

    def file_exists(self, project, packge, file):
